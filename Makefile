...
go-ut go/combined.coverprofile: go/vendor/.up-to-date $(GO_FILES)
	$(MAKE) golang-build-image
	find go/ -name "*.coverprofile" -type f -delete
	$(DOCKER_RUN) \
	    --net=host \
	    -v $${PWD}:/go/src/github.com/projectcalico/felix:rw \
	    -v $$HOME/.glide:/.glide:rw \
	...
	echo "mode: count" > "go/combined.coverprofile"
	cat `find go/ -name "*.coverprofile" -type f` | grep -v 'mode:' >> go/combined.coverprofile
